...
class BatteryPercentageLED(Node):
    ...
    def changeLED(self, percentage: float):
        ...
        if percentage <= 0.33:
            ...
            to_publish = [self.cp.pink, self.cp.pink, self.cp.pink,
                          self.cp.pink, self.cp.pink, self.cp.pink]
        elif percentage <= 0.67:
            # Change ALL 6 LEDs to Yellow
            to_publish = [self.cp.purple, self.cp.purple, self.cp.purple,
                          self.cp.purple, self.cp.purple, self.cp.purple]
        elif percentage <= 1.0:
            # Change ALL 6 LEDs to Green
            to_publish = [self.cp.cyan, self.cp.cyan, self.cp.cyan,
